                  ...
                  const spx_ctx *ctx, uint32_t addr[8])
{
    ...
    if (leaf_idx & 1) {
        memcpy(node_buf + PH_N, leaf, PH_N);
        memcpy(node_buf, path_ptr, PH_N);
    } else {
        memcpy(node_buf, leaf, PH_N);
        memcpy(node_buf + PH_N, path_ptr, PH_N);
    }
    path_ptr += PH_N;
    ...
    for (layer_idx = 0; layer_idx < tree_height - 1; layer_idx++) {
        ...
        if (leaf_idx & 1) {
            thash(node_buf + PH_N, node_buf, 2, ctx, addr);
            memcpy(node_buf, path_ptr, PH_N);
        } else {
...
{
    PH_VLA(uint8_t, stack, (tree_height+1)*PH_N);
    PH_VLA(unsigned int, node_heights, tree_height+1);
    ...
    for (leaf_pos = 0; leaf_pos < (uint32_t)(1 << tree_height); leaf_pos++) {
        ...
        gen_leaf(stack + stack_pos*PH_N, ctx, leaf_pos + idx_offset, tree_addr);
        stack_pos++;
        node_heights[stack_pos - 1] = 0;

        /* Check if this leaf is needed for auth path */
        if ((leaf_idx ^ 0x1) == leaf_pos) {
            memcpy(auth_path, stack + (stack_pos - 1)*PH_N, PH_N);
        }
        ...
        while (stack_pos >= 2 && node_heights[stack_pos - 1] == node_heights[stack_pos - 2]) {
            ...
            thash(stack + (stack_pos - 2)*PH_N,
                  stack + (stack_pos - 2)*PH_N, 2, ctx, tree_addr);
            stack_pos--;
            node_heights[stack_pos - 1]++;

            /* Check if parent node is needed for auth path */
            if (((leaf_idx >> node_heights[stack_pos - 1]) ^ 0x1) == parent_idx) {
                memcpy(auth_path + node_heights[stack_pos - 1]*PH_N,
                       stack + (stack_pos - 1)*PH_N, PH_N);
            }
        }
    }
    memcpy(root, stack, PH_N);
}