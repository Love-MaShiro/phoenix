...
| PH_GWOTSC_W1 | 16                 | 32                  |
| PH_GWOTSC_W2 | 32                 | 64                  |
| PH_GWOTSC_W1_LEN | 39             | 66                  |
| PH_GWOTSC_W2_LEN | 20             | 9                   |
...
**内存布局分析**:
- `pk_buf[4 * PH_GWOTSC_BYTES]` 包含4个叶子的所有链值
- `gwotsc_stride = PH_GWOTSC_BYTES`（每个叶子的字节数）
- `chain_buf += PH_N` 在循环中前进到下一个链位置
- 第 lane 个叶子的第 chain_idx 个链值位置：`pk_buf + lane*PH_GWOTSC_BYTES + chain_idx*PH_N`

**正确性验证**:
✅ PRF 和 thash 调用使用正确的 lane 偏移 (`lane_idx * gwotsc_stride`)
✅ 链迭代使用正确的地址配置（chain_idx, step_idx）
✅ 最终压缩使用 `inblocks=PH_GWOTSC_LEN=76`，通过通用 shake256x4 路径处理
...
| 项目 | 状态 | 说明 |
|------|------|------|
| gwotscx4 PRF调用 | ✅ 正确 | 使用正确的 lane 偏移 |
| gwotscx4 thash调用 | ✅ 正确 | 链迭代和地址配置正确 |
| gwotscx4 最终压缩 | ✅ 正确 | inblocks=76 通过通用路径 |
| octopus_compute_auth_pathsx4 | ✅ 正确 | 4路并行调用正确 |
| octopus_recompute_root | ✅ 正确 | 单路验证与并行签名等价 |
| thashx4 输入输出别名 | ⚠️ 注意 | 安全但应文档化 |

**结论**: GWOTSC 和 TFORS 的4路并行调用实现正确，与单路实现功能等价。
