...
//                         const spx_ctx *ctx)
// {
//     // 彻底不使用 pub_seed，只依赖 msg_digest
//     // (void)ctx;
//     printf("haraka shixian h2");
...
//     unsigned char hash_out[SPX_N];
//     uint8_t s_inc[65];

//     haraka_S_inc_init(s_inc);
//     haraka_S_inc_absorb(s_inc, msg_digest, SPX_N, ctx);
//     haraka_S_inc_finalize(s_inc);
//     haraka_S_inc_squeeze(hash_out, SPX_N, s_inc, ctx);

//     uint32_t per_block = (SPX_N * 8) / log2_kp;
//     if (per_block == 0) per_block = 1;

//     uint64_t val = 0;
//     size_t bytes = (SPX_N < sizeof(uint64_t)) ? SPX_N : sizeof(uint64_t);
...
//     unsigned char prev[SPX_N];
//     memcpy(prev, hash_out, SPX_N);

//     while (collected < m) {
//         haraka_S_inc_init(s_inc);
//         haraka_S_inc_absorb(s_inc, prev, SPX_N, ctx);
//         haraka_S_inc_finalize(s_inc);
//         haraka_S_inc_squeeze(hash_out, SPX_N, s_inc, ctx);
//         memcpy(prev, hash_out, SPX_N);
