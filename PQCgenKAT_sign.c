...
main(void)
{
    ...
    unsigned long long  mlen, smlen, mlen1, slen, tfslen;
    int                 count;
    int                 done;
    ...
    do {
        ...
        if ( (ret_val = crypto_sign(sm, &smlen, &slen, &tfslen, m, mlen, sk)) != 0) {
        ...
        if ( (ret_val = crypto_sign_open(m1, &mlen1, &slen, &tfslen, sm, smlen, pk)) != 0) {
